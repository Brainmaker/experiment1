...
from core import (
    dtype_cast, dropout, dropout_2, Dense, WordEncodeLayer, OutputLayer, BiLSTMEncodeLayer, LSTMDecodeLayer)
...
class Encoder(SubModule):
    def __init__(self, enc_dim, use_dropout):
        SubModule.__init__(self, ['enc_1', 'enc_2'])
    ...
    def encode(self, emb_x, x_mask):
        ...
        return tensor.concatenate([fh2[0], bh2[0]], axis=1)
...
class Decoder(SubModule):
    def __init__(self, dec_dim, use_dropout):
        SubModule.__init__(self, ['dec_1', 'dec_2'])
        self.use_dropout = use_dropout
    ...
    def _get_init_state(self, batch_size):
        return [tensor.alloc(dtype_cast(0.), batch_size, self.dec_dim) for i in range(6)]

    def _step(self, mask, y_tm1, _, c1_tm1, c2_tm1, h1_tm1, h2_tm1, context_vector):
        ...
        if self.use_dropout:
            ...
            h2, c2 = self.layers['dec_2'].forward(dropout_2(h1, shape), emb_y_tm1, mask, h2_tm1, c2_tm1, context_vector)
...
class WordDecoder(Decoder):
    def __init__(self, dec_dim, emb_size, word_emb_layer, use_dropout):
        Decoder.__init__(self, dec_dim, use_dropout)
        self.emb_size = emb_size
        self.layers['output_layer'] = OutputLayer(self.dec_dim, self.emb_size)
        self.word_emb_layer = word_emb_layer

    def _get_init_state(self, batch_size):
        init_state = [tensor.alloc(dtype_cast(0.), batch_size, self.emb_size),
                      tensor.alloc(dtype_cast(0.), batch_size, self.emb_size)]
        init_state += [tensor.alloc(dtype_cast(0.), batch_size, self.dec_dim) for i in range(4)]
        return init_state
    ...
    def _step(self, mask, y_tm1, _, c1_tm1, c2_tm1, h1_tm1, h2_tm1, context_vector):
        ...
        shape = (batch_size, self.dec_dim)
        if self.use_dropout:
            h1, c1 = self.layers['dec_1'].forward(dropout_2(emb_y_tm1, shape), emb_y_tm1, mask, h1_tm1, c1_tm1, context_vector)
            h2, c2 = self.layers['dec_2'].forward(dropout_2(h1, shape), emb_y_tm1, mask, h2_tm1, c2_tm1, context_vector)
            y = self.layers['output_layer'].forward(y_tm1, dropout_2(h2, shape), context_vector)

        else:
            h1, c1 = self.layers['dec_1'].forward(emb_y_tm1, emb_y_tm1, mask, h1_tm1, c1_tm1, context_vector)
            h2, c2 = self.layers['dec_2'].forward(h1, emb_y_tm1, mask, h2_tm1, c2_tm1, context_vector)
            y = self.layers['output_layer'].forward(y_tm1, h2, context_vector)
